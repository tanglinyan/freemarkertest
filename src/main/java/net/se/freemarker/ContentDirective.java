...
public class ContentDirective implements TemplateDirectiveModel {
    ...
    public void execute(Environment environment, Map map, TemplateModel[] templateModels, TemplateDirectiveBody templateDirectiveBody) throws TemplateException, IOException {
        if (templateDirectiveBody == null) {
            throw new TemplateModelException("templateDirectiveBody null");
        } else {
            String name = getString(PARAM_NAME, map);
            Integer age = getInt(PARAM_AGE, map);
            //接收到参数之后可以根据做具体的操作，然后将数据再在页面中显示出来。
            if (name != null) {
                environment.setVariable("output", DEFAULT_WRAPPER.wrap("//从ContentDirective解析类中获得的参数是：" + name + ", "));
            }

            if (age != null) {
                environment.setVariable("append", DEFAULT_WRAPPER.wrap("//年龄：" + age));
            }
